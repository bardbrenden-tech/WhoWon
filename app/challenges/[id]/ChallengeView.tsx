...
export default function ChallengeView({ challenge, sortedGames, standings, gameNames, userId }: Props) {
  ...
  const allDone = sortedGames.every(cg => cg.tournaments?.status === 'completed')
  const champion = allDone && standings.length > 0 ? standings[0] : null

  async function startTournament(cg: GameEntry) {
    if (!userId) return
    setStarting(cg.id)
    ...
    const { data: tournament, error } = await supabase
      ...
      .select('id').single()
    if (error || !tournament) { setStarting(null); return }
    ...
    const { data: tPlayers } = await supabase
      ...
      .select('id, seed')
    if (!tPlayers) { setStarting(null); return }
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Games */}
      <div>
        <h2 className="font-bold text-gray-900 mb-3">{tc.pickGames}</h2>
        <div className="space-y-3">
          {sortedGames.map((cg, i) => {
            ...
            return (
              <div key={cg.id} className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {status === 'notStarted' && tc.notStarted}
                    {status === 'ongoing' && tc.ongoing}
                    {status === 'done' && tc.done}
                  </p>
                </div>
                <div className="shrink-0">
                  {status === 'notStarted' && isCreator && (
                    <button
                      onClick={() => startTournament(cg)}
                      disabled={starting === cg.id}
                  ...
                  {status === 'done' && (
                    <span className="ml-2 text-green-600 text-sm font-semibold">✓</span>
                  )}
                </div>
              </div>
            )
      ...
      <div>
        ...
        <p className="text-xs text-gray-400 mt-2 text-center">
          1. plass = {getPoints(1)} · 2. = {getPoints(2)} · 3/4. = {getPoints(3)} · 5–8. = {getPoints(5)} · øvrige = {getPoints(9)} {tc.points}
        </p>
      </div>
    </div>
  )
