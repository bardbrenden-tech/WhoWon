...
export default async function ProfilePage() {
  ...
  const [{ data: profile }, { data: ratings }, { data: guestPlayers }, { data: recentSessions }] = await Promise.all([
    ...
    supabase.from('guest_players').select('id, name, email, avatar_color').eq('owner_id', user.id).order('name'),
    ...
    <div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-6 flex items-center gap-4">
          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-white text-2xl font-black flex items-center justify-center shrink-0 shadow-md shadow-indigo-200">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {t.profile.memberSince} {new Date(profile?.created_at ?? user.created_at).toLocaleDateString()}
            </p>
          </div>
        </div>
        ...
        {ratings && ratings.length > 0 && (
          <div className="bg-white border border-gray-200 rounded-2xl p-5 mb-6">
            <h2 className="font-bold text-gray-900 mb-4">📊 {t.profile.yourRatings}</h2>
                ...
                return (
                  <div key={r.id} className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <Link href={`/games/${r.game_id}`} className="text-sm font-semibold text-gray-800 hover:text-indigo-600">
                        {game?.name ?? r.game_id}
                      </Link>
                      <p className="text-xs text-gray-400">{r.games_played} {t.profile.games} · {r.wins} {t.profile.wins}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-black text-indigo-600 text-lg">{r.rating}</p>
                      <p className="text-xs text-gray-400">{t.profile.elo}</p>
                    </div>
                  </div>
                )
              })}
        ...
        {/* Guest players */}
        <div className="bg-white border border-gray-200 rounded-2xl p-5 mb-6">
          <h2 className="font-bold text-gray-900 mb-4">👥 {t.profile.yourPlayers}</h2>
          <GuestPlayersSection initialPlayers={guestPlayers ?? []} />
        </div>

        {/* Recent sessions */}
        {recentSessions && recentSessions.length > 0 && (
          <div className="bg-white border border-gray-200 rounded-2xl p-5">
            <h2 className="font-bold text-gray-900 mb-4">🕹️ {t.profile.recentGames}</h2>
